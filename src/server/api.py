...
def get_move(request: FenRequest):
    ...
    if visit_counts is not None and len(visit_counts) > 0:
        ...
        for idx in sorted_indices[:5]:
            count = float(visit_counts[idx])
            prob = float(probs[idx])
            if count > 0 or prob > 0.01:
                move_obj = action_indexer.idx_to_action(idx)
                move_uci = move_obj.uci() if isinstance(move_obj, chess.Move) else str(move_obj)
                top_moves.append({
                    "move": move_uci,
                    "visit_count": count,
                    "probability": prob
