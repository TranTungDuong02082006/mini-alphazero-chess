...
if __name__ == "__main__":
    ...
    net = NeuralNet(device="cpu")
    indexer = UCIActionIndexer()
    mcts = MCTS(network=net, action_indexer=indexer, num_simulations=args.sims)

    # Replay buffer
    buffer = ReplayBuffer(max_size=50000)

    # Self-play
    sp = SelfPlay(mcts, buffer, action_indexer=indexer)
    game_data = sp.play_game(max_moves=20)
    buffer.extend(game_data)

    print(f"Replay buffer size after one game: {len(buffer)}")